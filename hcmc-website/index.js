document
  ...
  .addEventListener("click", function (event) {
    ...
    [patientIdNumber, firstName, lastName, dateOfBirth, ddlDepartment].forEach(
      (input) => {
        if (
          !input.value ||
    ...
    if (!isValid) {
      alert("Please fill in all required fields with the correct format.");
      return;
    }

    let ageMilliSeconds = Date.now() - new Date(dateOfBirth.value);
    let ageDate = new Date(ageMilliSeconds);
    let isOld = Math.abs(ageDate.getUTCFullYear() - 1970) > 65 ? true : false;
